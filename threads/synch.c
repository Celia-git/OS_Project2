...
static void donate_priority_chain(struct thread *t);
...
/* Donation chain propagation */
static void
donate_priority_chain(struct thread *t)
{
  int depth = 0;

  while (t->waiting_lock != NULL && depth < DONATION_MAX_DEPTH)
    {
...
/* Acquire lock with donation logic */
void
